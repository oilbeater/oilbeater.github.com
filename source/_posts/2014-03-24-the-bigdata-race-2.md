...
具体思路就是看每个用户每个月都买了哪些物品，如果有一个用户（或者可以自己改阈值）在任意两个月内购买了同一物品，那么就把这个物品标记为周期性物品。遍历完所有用户找到所有的周期性物品。如果用户购买过某个周期性物品的话就让他下个月再买一遍好了。
...
对所有用户的购买序列进行这种操作，得到所有的二元组并统计每个二元组出现的次数。比如所有二元组是(a, b), (a, c), (b, c)，(a, c), (b, c),  (b, c)，得到的结果就是{(a, b) : 1, (a, c) : 2, (b, c) : 3}。现在就得到了买过每种物品后续又买了哪个物品的次数。
